...
test = datetime.time()
outputfile = openpyxl.load_workbook(r"C:\1\【20210510実績】ダイエットクック日別勤怠集計表奥山ver2.xlsx")
...
for i in list83:
    ...
    if i[4] == "YAKI":
        ...
        OutPut__sheets.cell(row=row, column=6).value = "一次処理"  # dep
        OutPut__sheets.cell(row=row, column=5).value = ""  # li-ne
    else:
...
for i in list11:
    ...
    if i[2] == 5030:
        OutPut__sheets.cell(row=row, column=11).value = "〇"
    else:
        pass
